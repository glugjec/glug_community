...
export default function AdminDashboard() {
  ...
  // Tab guard: if user is content_admin, only allow events and resources
  useEffect(() => {
    ...
    <section className="admin-page">
      ...
      <header className="admin-header">
        <div className="admin-header-main">
          ...
          <h1 className="admin-title">
            {isContentAdmin ? "GLUG Content Management" : "GLUG Administration Console"}
          </h1>
          <p className="admin-subtitle">
            {isContentAdmin
              ? "Curate curriculum learning resources, upload code packages, publish upcoming events, and manage workshop galleries."
      ...
      {isContentAdmin ? (
        <div className="admin-stats-grid">
          ...
          >
            <div className="admin-stat-header">
              <span className="admin-stat-label">Featured Topics</span>
              <div className="admin-stat-icon-wrap post-theme">
                <Star size={18} />
              </div>
            </div>
            <div className="admin-stat-body">
              <span className="admin-stat-value">{resources.filter((r) => r.isFeatured).length}</span>
              <span className="admin-stat-subtext">Highlighted for learners</span>
            </div>
          </div>
        </div>
      ) : (
          ...
          {modSubTab === "appeals" && (() => {
            ...
            return (
              <>
                ...
                <div className="admin-table-container admin-appeals-table-wrap">
                  <table className="admin-table">
                    <thead>
                      <tr>
                        <th>Appellant</th>
                ...
                <div className="admin-appeals-mobile-list">
                  {loadingAppeals && appeals.length === 0 ? (
                    <div className="admin-appeal-card empty">
                      <RefreshCw className="admin-spin" size={20} />
                      <span>Loading appeals...</span>
                    </div>
                  ) : filteredAppeals.length === 0 ? (
                    ...
                    filteredAppeals.map((a) => {
                      const isAccBan = a.itemType === "account_ban" || a.originalCategory === "account_ban";
                      return (
                        <div key={a.id || a._id} className="admin-appeal-card">
                          <div className="admin-appeal-card-header">
                            <div className="admin-user-cell">
                              <AdminUserAvatar
                                src={a.appellant?.avatar}
                                name={a.appellant?.username || "User"}
                          ...
                          <div className="admin-appeal-card-body">
                            <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
                              {isAccBan ? (
                                <span
                                  className="admin-badge danger"
                            ...
                            <div className="admin-appeal-reason-box">
                              ...
                              <div
                                style={{
                                  maxHeight: expandedTextMap[`appeal-reason-m-${a.id || a._id}`] ? "none" : "60px",
                                  overflow: expandedTextMap[`appeal-reason-m-${a.id || a._id}`] ? "visible" : "hidden",
                                  position: "relative",
                                  transition: "max-height 0.2s ease",
                                }}
                              >
                              ...
                              {a.originalReason && a.originalReason.length > 80 && (
                                <button
                                  type="button"
                                  onClick={() => toggleTextExpanded(`appeal-reason-m-${a.id || a._id}`)}
                                  style={{
                                    background: "none",
                                    border: "none",
                                    color: "#60a5fa",
                                ...
                                >
                                  {expandedTextMap[`appeal-reason-m-${a.id || a._id}`] ? "Show less" : "Show more"}
                                </button>
                              )}
                            </div>

                            <div className="admin-appeal-statement-box">
                              <span style={{ fontSize: "0.74rem", fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.03em" }}>
                                Appellant Statement
                              </span>
                              <div
                                style={{
                                  maxHeight: expandedTextMap[`appeal-m-${a.id || a._id}`] ? "none" : "86px",
                                  overflow: expandedTextMap[`appeal-m-${a.id || a._id}`] ? "visible" : "hidden",
                                  position: "relative",
                                  transition: "max-height 0.2s ease",
                                }}
                              >
                              ...
                              {a.statement && a.statement.length > 100 && (
                                <button
                                  type="button"
                                  onClick={() => toggleTextExpanded(`appeal-m-${a.id || a._id}`)}
                                  style={{
                                    background: "none",
                                    border: "none",
                                    color: "#60a5fa",
                                ...
                                >
                                  {expandedTextMap[`appeal-m-${a.id || a._id}`] ? "Show less" : "Show more"}
                                </button>
                              )}
                              <span className="admin-date-subtext" style={{ marginTop: "4px" }}>
                                Submitted {new Date(a.createdAt).toLocaleDateString()}
                              </span>
                            </div>
                          ...
                          <div className="admin-appeal-card-actions">
                            <button
                              type="button"
                              className="admin-action-btn"
                              onClick={() => {
                          ...
                          {a.status !== "pending" && a.resolvedBy && (
                            <span className="admin-date-subtext" style={{ textAlign: "right", marginTop: "2px" }}>
                              Resolved by @{a.resolvedBy}
                            </span>
                          )}
                        </div>
      ...
      {roleChangeTarget && (
        <div className="admin-modal-overlay" onClick={() => !isUpdatingRole && setRoleChangeTarget(null)}>
          <div className="admin-modal-box alert" style={{ maxWidth: "440px" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px", margin: "16px 0", textAlign: "left" }}>
              <label
                style={{
                  display: "flex",
                  ...
                  padding: "10px 12px",
                  borderRadius: "8px",
                  border: targetRole === "student" ? "1px solid #3b82f6" : "1px solid rgba(255, 255, 255, 0.1)",
                  background: targetRole === "student" ? "rgba(59, 130, 246, 0.1)" : "rgba(255, 255, 255, 0.03)",
                  cursor: "pointer",
                }}
              >
                <input
                ...
                />
                <div>
                  <div style={{ fontWeight: 600, fontSize: "14px", color: "#f8fafc" }}>Student Member</div>
                  <div style={{ fontSize: "12px", color: "#94a3b8" }}>Regular community member with forum and learning access</div>
                </div>
              </label>
              ...
              <label
                style={{
                  ...
                  padding: "10px 12px",
                  borderRadius: "8px",
                  border: targetRole === "content_admin" ? "1px solid #06b6d4" : "1px solid rgba(255, 255, 255, 0.1)",
                  background: targetRole === "content_admin" ? "rgba(6, 182, 212, 0.1)" : "rgba(255, 255, 255, 0.03)",
                  cursor: "pointer",
                }}
              >
                <input
                ...
                />
                <div>
                  <div style={{ fontWeight: 600, fontSize: "14px", color: "#38bdf8" }}>Content Administrator</div>
                  <div style={{ fontSize: "12px", color: "#94a3b8" }}>Can create and manage Events and Resources only</div>
                </div>
              </label>
              ...
              <label
                style={{
                  ...
                  padding: "10px 12px",
                  borderRadius: "8px",
                  border: targetRole === "admin" ? "1px solid #f59e0b" : "1px solid rgba(255, 255, 255, 0.1)",
                  background: targetRole === "admin" ? "rgba(245, 158, 11, 0.1)" : "rgba(255, 255, 255, 0.03)",
                  cursor: "pointer",
                }}
              >
                <input
                ...
                />
                <div>
                  <div style={{ fontWeight: 600, fontSize: "14px", color: "#fbbf24" }}>Full Administrator</div>
                  <div style={{ fontSize: "12px", color: "#94a3b8" }}>Full access to users, moderation, team, and system settings</div>
                </div>
              </label>
            </div>
